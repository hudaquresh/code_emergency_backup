...
nc_fid = Dataset("../data/synthetic-storm-data/CHAZ_knaff15_Mumbai.nc", 'r')
nc_attrs, nc_dims, nc_vars = ncdump(nc_fid)
      
data = [["CHAZ_knaff15_Mumbai.nc", "CHAZ NC", "other_chaz", "r", 32.0*120*27]]

     
data_path = os.path.join("../data", "synthetic-storm-data", data[0][0])
storms = load_other_chaz_storms(path = data_path,
                mask_distance = None,
...
axes.plot(test_storm.max_wind_speed, test_storm.max_wind_radius/1000, 'ro', label="Knaff Max Wind Radius") 
...
units.convert(test_storm.max_wind_radius, 'km', 'nmi'), 'ro-', label="Knaff MWR")  
...
axes.set_xlabel('Obs Count', **axis_font)
axes.set_ylabel('Max Wind Radius (meters)', **axis_font)
axes.legend()

plt.yticks(fontsize=10)
plt.xticks(fontsize=10)
plt.tight_layout()
plt.savefig('knaff_CHAZ_mwr_check.pdf')
